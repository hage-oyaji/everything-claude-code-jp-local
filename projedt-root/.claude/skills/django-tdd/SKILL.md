---
name: django-tdd
description: pytest-djangoによるDjangoテスト戦略、TDD方法論、factory_boy、モック、カバレッジ、Django REST Framework APIのテスト。
origin: ECC
---
...
- **ファクトリを使用**: 手動のオブジェクト作成の代わりに
- **テストごとにアサーション1つ**: テストを焦点を絞る
...
- **Djangoの内部をテストしない**: Djangoの動作を信頼する
- **サードパーティコードをテストしない**: ライブラリの動作を信頼する
- **失敗するテストを無視しない**: すべてのテストがパスすること
- **テストを依存させない**: テストは任意の順序で実行可能に
- **過剰にモックしない**: 外部依存関係のみモック

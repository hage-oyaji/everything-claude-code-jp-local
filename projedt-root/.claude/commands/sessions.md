---
description: Claude Codeのセッション履歴、エイリアス、セッションメタデータを管理します。
---

# セッションコマンド

Claude Codeのセッション履歴を管理します — `~/.claude/session-data/`に保存されたセッションの一覧表示、ロード、エイリアス設定、編集を行い、レガシーの`~/.claude/sessions/`からも読み取ります。
...
- セッションファイルはヘッダーに`Project`、`Branch`、`Worktree`を保持しているため、`/sessions info`で並列tmux/ワークツリー実行を区別できます。
- コマンドセンタースタイルの監視には、`/sessions info`、`git diff --stat`、および`scripts/hooks/cost-tracker.js`が出力するコストメトリクスを組み合わせてください。

## 引数

$ARGUMENTS:
- `list [options]` - セッション一覧
  - `--limit <n>` - 最大表示セッション数（デフォルト：50）
  - `--date <YYYY-MM-DD>` - 日付でフィルタ
  - `--search <pattern>` - セッションIDで検索
- `load <id|alias>` - セッション内容のロード
...
- セッションはMarkdownファイルとして`~/.claude/session-data/`に保存され、レガシーの`~/.claude/sessions/`からも読み取ります
- エイリアスは`~/.claude/session-aliases.json`に保存されます
- セッションIDは短縮可能です（最初の4〜8文字で通常十分にユニーク）
